...
public class StatementGeneratorTest extends GenerationTest {
  ...
  public void testObjectArrayInitializer() throws IOException {
    ...
    assertEquals(1, stmts.size());
    result = generateStatement(stmts.get(0));
    assertEquals("IOSObjectArray *a = [IOSObjectArray arrayWithObjects:(id[]){ @\"one\", @\"two\", @\"three\" } count:3 type:IOSClass_fromProtocol(@protocol(CommonKotlinComparable))];", result);
  }
